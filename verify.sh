...
for targetSnapshotVersion in ${snapshotVersions}; do
  ...
  ./mvnw clean verify -Dspring-boot.version=${targetSnapshotVersion} -Denforcer.skip=true -Djacoco.skip=true ${options} && ./mybatis-spring-boot-samples/run_fatjars.sh && exitCode=0 || exitCode=$?
  if [ "${exitCode}" = "0" ]; then
